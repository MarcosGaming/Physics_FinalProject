...
Mesh::Mesh(MeshType type)
{
	...
	std::vector<Vertex> temporalVector = std::vector<Vertex>(std::begin(vertices), std::end(vertices));
	bool uniqueFound = false;
	for (auto vertex1 = temporalVector.begin(); vertex1 != temporalVector.end(); ++vertex1)
	{
		for (auto vertex2 = m_vertices.begin(); vertex2 != m_vertices.end(); ++vertex2)
		{	
		...
		if (uniqueFound || m_vertices.empty())
		{
			Vertex uniqueVertex = Vertex(glm::vec3(vertex1->getCoord()));
			m_vertices.push_back(uniqueVertex);
		}
	}
}
